import pathlib
import warnings

import numpy as np
import pytest
import xobjects as xo
from scipy.integrate import IntegrationWarning
from xobjects.test_helpers import fix_random_seed

import xpart as xp
import xtrack as xt


test_data_folder = pathlib.Path(__file__).parent.joinpath('../test_data').absolute()


def _generate_matched_gaussian_bunch(line, sigma_z):
    with warnings.catch_warnings():
        warnings.filterwarnings(
            'ignore',
            message='The integral is probably divergent, or slowly convergent.',
            category=IntegrationWarning,
        )
        return xp.generate_matched_gaussian_bunch(
            line=line,
            num_particles=100_000,
            nemitt_x=2e-6,
            nemitt_y=2e-6,
            sigma_z=sigma_z,
        )


@pytest.mark.parametrize('shift_zeta', [-0.4, 0, 0.4])
@pytest.mark.parametrize('sigma_z', [12e-2, 1.2e-2])
@fix_random_seed(12345)
def test_stationary_bucket_and_bunch_matching(shift_zeta, sigma_z):
    env = xt.load([
        test_data_folder / 'sps_thick/sps.seq',
        test_data_folder / 'sps_thick/lhc_q20.str',
    ])
    line = env.sps
    line.set_particle_ref('proton', p0c=26e9)

    line.append('timedelay', xt.TimeDelay(shift_zeta=shift_zeta))

    env['actcse.31632'].frequency = 200e6
    env['actcse.31632'].voltage = 4.5e6
    env['actcse.31632'].phase = np.pi

    tw = line.twiss6d()

    particles = _generate_matched_gaussian_bunch(line=line, sigma_z=sigma_z)

    rfb = line._get_bucket()

    z_separatrix = np.linspace(rfb.z_left, rfb.z_right, 1000)
    delta_separatrix = rfb.separatrix(z_separatrix)
    delta_separatrix_neg = rfb.separatrix(z_separatrix, sgn=-1)

    xo.assert_allclose(rfb.dp0, tw.delta[0], rtol=0, atol=1e-12)
    xo.assert_allclose(rfb.z_sfp, tw.zeta[0], rtol=1e-3, atol=1e-9)

    # For this stationary single-RF bucket, the bucket is centered at zeta0.
    xo.assert_allclose(
        0.5 * (rfb.z_left + rfb.z_right), tw.zeta[0], rtol=1e-3, atol=1e-10
    )

    # The stable fixed point Hamiltonian must be evaluated at the shifted
    # momentum coordinate dp0.
    xo.assert_allclose(
        rfb.h_sfp(),
        rfb.hamiltonian(rfb.z_sfp, rfb.dp0),
        rtol=0,
        atol=1e-12,
    )

    # The upper and lower separatrix branches are symmetric around dp0.
    xo.assert_allclose(
        (delta_separatrix + delta_separatrix_neg) / 2,
        rfb.dp0,
        rtol=0,
        atol=1e-14,
    )
    xo.assert_allclose(delta_separatrix[[0, -1]], rfb.dp0, rtol=0, atol=1e-12)
    xo.assert_allclose(delta_separatrix_neg[[0, -1]], rfb.dp0, rtol=0, atol=1e-12)

    assert rfb.is_in_separatrix(tw.zeta[0], tw.delta[0])
    assert np.all(rfb.is_in_separatrix(np.array(particles.zeta),
                                       np.array(particles.delta)))

    xo.assert_allclose(np.mean(particles.zeta), tw.zeta[0], rtol=0, atol=2e-3)
    xo.assert_allclose(np.std(particles.zeta), sigma_z, rtol=2e-2, atol=0)
    xo.assert_allclose(np.mean(particles.delta), tw.delta[0], rtol=0, atol=2e-5)

    z_probe = np.array([-0.25, 0, 0.25]) * (rfb.z_right - rfb.z_left)
    delta_probe_sep = rfb.separatrix(z_probe)

    # Launch three probe particles just inside the separatrix and three just
    # outside it, at the same zeta coordinates.
    delta_probe_inside = rfb.dp0 + 0.9 * (delta_probe_sep - rfb.dp0)
    delta_probe_outside = rfb.dp0 + 1.1 * (delta_probe_sep - rfb.dp0)
    z_probe_all = np.r_[z_probe, z_probe]
    delta_probe_all = np.r_[delta_probe_inside, delta_probe_outside]

    probe = line.build_particles(
        zeta=z_probe_all,
        delta=delta_probe_all,
        x=tw.dx[0] * delta_probe_all,
        px=tw.dpx[0] * delta_probe_all,
    )

    line.track(probe, num_turns=200, turn_by_turn_monitor=True)
    mon = line.record_last_track

    inside_is_accepted = rfb.is_in_separatrix(
        np.array(mon.zeta[:3, :]),
        np.array(mon.delta[:3, :]),
    )
    outside_is_accepted = rfb.is_in_separatrix(
        np.array(mon.zeta[3:, :]),
        np.array(mon.delta[3:, :]),
    )

    # The particles launched inside stay in the bucket, while the particles
    # launched outside drift away instead of being captured.
    assert np.all(inside_is_accepted)
    assert not np.any(outside_is_accepted)
    assert np.all(
        np.ptp(np.array(mon.zeta[:3, :]), axis=1) < (rfb.z_right - rfb.z_left)
    )
    assert np.all(
        np.ptp(np.array(mon.zeta[3:, :]), axis=1)
        > 3 * (rfb.z_right - rfb.z_left)
    )


@pytest.mark.parametrize('sigma_z', [2e-2, 2e-3])
@fix_random_seed(12345)
def test_bucket_with_radiation_and_bunch_matching(sigma_z):
    env = xt.load([
        test_data_folder / 'sps_thick/sps.seq',
        test_data_folder / 'sps_thick/lhc_q20.str',
    ])
    line = env.sps
    line.set_particle_ref('positron', p0c=30e9)

    line.cycle('qf.30010')

    env['actcse.31632'].frequency = 200e6
    env['actcse.31632'].voltage = 100e6
    env['actcse.31632'].phase = np.pi

    line.configure_radiation(model='mean')
    line.compensate_radiation_energy_loss()
    tw_rad = line.twiss6d()

    particles = _generate_matched_gaussian_bunch(line=line, sigma_z=sigma_z)

    rfb = line._get_bucket()

    z_separatrix = np.linspace(rfb.z_left, rfb.z_right, 1000)
    delta_separatrix = rfb.separatrix(z_separatrix)
    delta_separatrix_neg = rfb.separatrix(z_separatrix, sgn=-1)

    xo.assert_allclose(rfb.z_sfp, tw_rad.zeta[0], rtol=0, atol=1e-12)
    xo.assert_allclose(rfb.dp0, tw_rad.delta[0], rtol=0, atol=1e-12)

    # Here dp0 is generated by radiation compensation, not by a deliberately
    # imposed zeta shift.
    assert np.abs(rfb.dp0) > 1e-4
    xo.assert_allclose(rfb.z_sfp, 0, rtol=0, atol=1e-4)

    # The stable fixed point Hamiltonian must be evaluated at the shifted
    # momentum coordinate dp0.
    xo.assert_allclose(
        rfb.h_sfp(),
        rfb.hamiltonian(rfb.z_sfp, rfb.dp0),
        rtol=0,
        atol=1e-12,
    )

    # The upper and lower separatrix branches are symmetric around dp0.
    xo.assert_allclose(
        (delta_separatrix + delta_separatrix_neg) / 2,
        rfb.dp0,
        rtol=0,
        atol=1e-14,
    )
    xo.assert_allclose(delta_separatrix[[0, -1]], rfb.dp0, rtol=0, atol=1e-9)
    xo.assert_allclose(delta_separatrix_neg[[0, -1]], rfb.dp0, rtol=0, atol=1e-9)

    assert rfb.is_in_separatrix(tw_rad.zeta[0], tw_rad.delta[0])
    assert np.mean(
        rfb.is_in_separatrix(np.array(particles.zeta), np.array(particles.delta))
    ) > 0.99

    # The accelerating bucket makes the bunch slightly asymmetric.
    xo.assert_allclose(np.mean(particles.zeta), tw_rad.zeta[0], rtol=0, atol=4e-3)
    xo.assert_allclose(np.std(particles.zeta), sigma_z, rtol=2e-2, atol=0)
    xo.assert_allclose(np.mean(particles.delta), tw_rad.delta[0], rtol=0, atol=2e-5)

    z_sfp = np.atleast_1d(rfb.z_sfp)[0]
    z_probe = z_sfp + np.array([-0.2, 0, 0.2]) * (rfb.z_right - rfb.z_left)
    delta_probe_sep = rfb.separatrix(z_probe)

    # Launch three probe particles close to the separatrix from the inside and
    # three outside it, at the same zeta coordinates.
    delta_probe_inside = rfb.dp0 + 0.85 * (delta_probe_sep - rfb.dp0)
    delta_probe_outside = rfb.dp0 + 1.2 * (delta_probe_sep - rfb.dp0)
    z_probe_all = np.r_[z_probe, z_probe]
    delta_probe_all = np.r_[delta_probe_inside, delta_probe_outside]

    probe = line.build_particles(
        zeta=z_probe_all,
        delta=delta_probe_all,
    )

    assert np.all(rfb.is_in_separatrix(probe.zeta[:3], probe.delta[:3]))
    assert not np.any(rfb.is_in_separatrix(probe.zeta[3:], probe.delta[3:]))

    line.track(probe, num_turns=200, turn_by_turn_monitor=True)
    mon = line.record_last_track

    inside_is_accepted = rfb.is_in_separatrix(
        np.array(mon.zeta[:3, :]),
        np.array(mon.delta[:3, :]),
    )
    outside_final_is_accepted = rfb.is_in_separatrix(
        np.array(mon.zeta[3:, -1]),
        np.array(mon.delta[3:, -1]),
    )

    # The particles launched inside stay in the bucket, while the particles
    # launched outside drift away by many bucket widths.
    assert np.all(inside_is_accepted)
    assert not np.any(outside_final_is_accepted)
    assert np.all(
        np.ptp(np.array(mon.zeta[:3, :]), axis=1) < (rfb.z_right - rfb.z_left)
    )
    assert np.all(
        np.ptp(np.array(mon.zeta[3:, :]), axis=1)
        > 3 * (rfb.z_right - rfb.z_left)
    )
